var compile = (function(){
	...
	var stdlib = (new Function('return ' + readFileSync(__dirname + '/stdlib.js').toString()))();

	var fest_ns = 'http://fest.mail.ru';
	var fest_i18n_ns = 'http://i18n.fest.mail.ru';

	var short_tags = stdlib.core.short_tags;
	var escapeJS = stdlib.core.escapeJS;
	var escapeHTML = stdlib.core.escapeHTML;
	...
	function _compile (data){
		...
		var counters = {counter: 0, promises: 1},
			...
			compile_file = translate({
				file: file,
				template: contents || readFileSync(file, 'utf-8'),
				sax: sax,
				fest_ns: fest_ns,
	...
	function finish_compile(file, options, name){
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'var __fest_self=this,' +
						...
						inlineSource(stdlib, 'core', options.std).join(',') + ',' +
						(output.json ? inlineSource(stdlib, 'json', options.std).join(',') + ',' : '') +
						(output.plural && options.plural ? (
							'__fest_plural = ' + options.plural.toString() + ',' +
							inlineSource(stdlib, 'plural', options.std).join('\n') + ','
						) : '') +
		...
		if (file && file.path && file.contents) {
	...
	function inlineSource(object, section, useStd) {
		var exports = [];
		var target = object[section];
		...
		for (var name in target) {
			if (target.hasOwnProperty(name)) {
				...
				if (useStd) {
					value = stdlib.namespace + '.' + section + '.' + name;
				} else {
				...
				exports.push('__fest_' + name + ' = ' + value);
			}
		}
